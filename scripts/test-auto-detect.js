...
const AGENTS = ["models/antigravity-preview-05-2026", "models/deep-research-preview-04-2026", "models/deep-research-max-preview-04-2026"];
...
assert(flash && flash.inputTokenLimit === 1048576 && flash.outputTokenLimit === 65536, "token limits read from verified indices [5]/[6]");
assert(flash && Array.isArray(flash.supportedGenerationMethods), "methods exposed on kept models");
assert(!r1.kept.includes("models/gemini-3.5-live-translate-preview"), "bidi-only live model filtered (no routable method)");
assert(!r1.kept.includes("models/veo-3.1-generate-preview"), "predictLongRunning video model filtered (no routable method)");
...
const shifted = JSON.stringify([[[...agentEntry.slice(0, 1), "SHIFT", ...agentEntry.slice(1)], [...normalEntry.slice(0, 1), "SHIFT", ...normalEntry.slice(1)]]]);
...
assert(!r3.kept.includes("models/gemma-4-31b-it") && r3.filtered.some(f => f.startsWith("models/gemma-4-31b-it [blocklist]")), "env blocklist filters configured pattern only");

// ---- Batch 4: strict=false exposes everything ----
process.env.DYNAMIC_MODELS_STRICT = "false";
const r4 = run(realText);
process.env.DYNAMIC_MODELS_STRICT = "true";
const realEntryCount = orig.filter(e => Array.isArray(e) && e.length > 3 && typeof e[0] === "string" && String(e[0]).startsWith("models/")).length;
assert(r4.kept.length === realEntryCount && !r4.filtered.length, `strict=false exposes all ${realEntryCount} entries unfiltered`);
