...
3. **配置应用**：填写以下参数：
   ...
   **Usage**:
   ...
   > 💡 **提示**：如果同时登录多个账号（`MAX_CONTEXTS` > 1），可能需要增加内存配置。
...
部署后，需要通过上传认证文件来添加 Google 账号：

**上传认证文件**

- 在本地机器上运行 `npm run extract-auth` 提取认证文件（参考主 README 中的 [直接运行](../../README.md#-直接运行windows--macos--linux) 的 1 至 3），认证文件在 `configs/auth`
